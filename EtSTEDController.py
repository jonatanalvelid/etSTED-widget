...
class EtSTEDController(ImConWidgetController):
    ...
    def endRecording(self):
        ...
        savename = getUniqueName(name)  #TODO: not called getUniqueName anymore, does it still exist a function with the same functionality?
        ...
        if name == self.detectorFast:
            if self.__binary_stack is None:
                ...
                if np.ndim(self.__binary_stack) == 2:
                    ...
                    print(np.shape(img))
                    print(np.shape(self.__binary_stack))
